...
def list_folder(folder_path='/'):
    ...
    api_url = 'https://api.dropbox.com/1/metadata/dropbox/%s?list=true'
    escaped_path = urllib.parse.quote(folder_path.encode('utf-8'))
    escaped_url = api_url % escaped_path
    response = requests.get(escaped_url, headers=headers)
...
def download_file(path, dest_filename, progress=None):
    ...
    with open(dest_path, 'wb') as f:
        for chunk in response.iter_content(1024*10):
            ...
            if size > 0 and isinstance(progress, collections.Callable):
...
class DropboxView (ui.View):
    ...
    def load_folder(self):
        ...
        if not infos:
            import console
            console.alert('Error', 
                'Could not load folder. Please check if you entered the access token correctly.',
                'OK', 
        ...
        for info in infos:
            ...
            item = {'title': name, 
                'image': 'ionicons-folder-32' if is_dir else 'ionicons-ios7-download-outline-32',
                'accessory_type': 'disclosure_indicator' if is_dir else 'none',
                'is_dir': is_dir,
                'path': info['path']}
            items.append(item)
